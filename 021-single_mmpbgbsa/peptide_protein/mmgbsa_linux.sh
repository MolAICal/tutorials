...
molaical.exe -call run -c runnamd -i -dispdev text -args -s complex_final.psf -c complex_final.pdb -minimize 2000 -if_run 1 -output_prefix com

# 5. Cut last 10 steps for MM/GBSA calculation 
molaical.exe -call run -c catdcd -i -dispdev text -args -out result.dcd -first 1991 -last 2000 -stride 1 com_md.dcd

# 6. Calculate MM/GBSA
molaical.exe -call run -c mmpbgbsa -i -dispdev text -args -s complex_final.psf -c complex_final.pdb -t result.dcd -cs "protein" -rs "not,segname,$molargs3" -ls "segname,$molargs3" -pb 0 -gb 1
